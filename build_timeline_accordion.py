...
for p in paths:
    if os.path.exists(p):
        ...
        # Limpar possivel injecao de window.potencializar anterior pra evitar duplicatas se der regex maluca
        c = re.sub(r'window\.potencializarResultados\s*=\s*function\(\)\s*\{.*?\}\s*;?\s*', '', c, flags=re.DOTALL)
        
        # 1. Update getInitialState & globals
        c = re.sub(r'// Configurações e Persistência do App.*?function getInitialState\(\) \{.*?(?=\nlet state = getInitialState\(\);)', lambda x: new_functions + "\n" + new_initial_state, c, flags=re.DOTALL)
        
        # 2. Update renderHome
        c = re.sub(r'function renderHome\(\) \{.*?(?=\n// ------)', lambda x: new_render_home + "\n}", c, flags=re.DOTALL)
